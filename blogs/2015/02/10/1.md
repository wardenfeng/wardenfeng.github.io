...
为了加快访问速度，我的wordpress采用了使用阿里云的oss来保存附件。

安装与使用【阿里云附件】插件请查看[马文建的博客](http://mawenjian.net/p/1453.html)。

我安装了【阿里云附件v2.0 beta.zip】插件，使用时遇到了点bug。

![](http://images.feng3d.me/wordpress/wp-content/uploads/2015/02/20150210043916.png)

明明已经写好了上传到oss的wordpress/wp-content/upload路径下却把图片等资源上传到C:\wamp\www文件夹下了。

![](http://images.feng3d.me/wordpress/wp-content/uploads/2015/02/20150210044633.jpg)

为了解决这个问题，不懂php的我也豁出去了，因为太想要这个插件功能了。

 

经过半夜中5个小时的努力终于解决了，在oss-support.php文件中添加了个函数用来计算上传文件夹路径，并在需要的地方使用。

虽然代码不多，由于对php不熟，无法想as、java、c++等语言那样使用IDE断点调试，花费时间挺长的。。。

```
/**
 * 获取oss上传路径
 */
function get_object_path($metadatafile){
	
	//处理错误斜杠
	$object = str_replace("\\" ,'/' ,$metadatafile);
	//
	...
	//获取上传路径
	$object = str_replace($upload_path, '', $object);
	
	$upload_url_path = get_option('upload_url_path');
	//删除域名 如 http://images.feng3d.me/
	if(substr($upload_url_path,0,7) == "http://")
	{
		$upload_url_path = substr($upload_url_path,strpos($upload_url_path,"/",7)+1);
	}
	$object = $upload_url_path.$object;
	
	return $object;
}
```

以下是完整的oss-support.php文件
...
function upload_attachments($metadata) {
	...
	//在本地的存储路径
	$file = get_file_path($metadata['file']);
	
	//设置可选参数
	$opt =array('Content-Type' => $metadata['type']);
	
	//获取oss上传路径
	$object = get_object_path($metadata['file']);
	
	//执行上传操作
	_file_upload ( $object, $file, $opt);
	//_logged('2upload.txt',"object=$object;file=$file;upload_path=$upload_path;metadatafile=$metadatafile");
 ...
 * 获取本地绝对路径
 */
function get_file_path($metadatafile){
	$file = str_replace("\\" ,'/' ,$metadatafile);
	$file = str_replace(get_home_path(), '', $file);
	
	$file = get_home_path().$file;	//向上兼容，较早的WordPress版本上$metadata['file']存放的是相对路径
	return $file;
}
...
function upload_thumbs($metadata) {
	...
	if (isset($metadata['sizes']) && count($metadata['sizes']) > 0)
	{
		...
		{
			//生成object在OSS中的存储路径
			$object = get_object_path($object_path.$val['file']);
			//生成本地存储路径
...
{	
	...
	$del_file_path = get_object_path($del_file_path);
