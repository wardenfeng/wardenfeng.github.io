...
![](http://images.feng3d.me/wordpress/wp-content/uploads/2014/07/20140710222948-300x165.jpg)

[查看实例](http://images.feng3d.me/feng3dDemo/MD5MeshParserTest.html)

以下代码基本上都是away3d的md5mesh解析代码，我只是按自己的理解添加或者翻译了注释。
...
这里个as代码文件中，大部分做的是解析功能，但translateGeom函数却是最关键的代码，因为它处理的是蒙皮功能！

我理解这个函数的时候主要参考的是以下网址的内容，介绍的是MD5模型的格式、导入与顶点蒙皮式骨骼动画。

 http://blog.csdn.net/summerhust/article/details/17421213

 

md5mesh解析出来的几何体（SkinnedSubGeometry）比obj文件（SubGeometry）多两条数据，一个是关节索引数据（jointIndices），另一个是关节权重数据（jointWeights）。

索引数据与权重数据将会在蒙皮过程中使用到。